...
#c::
	...
	if (title <> "") {
		if (not notRemoveAmp) {
			noAmpTitle := removeAmp(title)
		}
		Clipboard := noAmpTitle
	}
	return
...
getFileList(ByRef clipFile) 
{
	...
	Loop, %count%
	{
		...
		If fileList=
			fileList=%filePath%
		Else
			fileList=%fileList%`r`n%filePath%
		filePath=
		utf8File=
	}
	fileClip=
...
; Whether the clipboard is stored file list. 
IsClipboardFileList() {
	clipFile=%A_Temp%\showClipboard.ahk.temp.clip.fl.YouCanDeleteItManually
	FileAppend, %ClipboardAll%, %clipFile%
	tmpClipFile := FileOpen(clipFile, "r")
	tmpClipFile.seek(12)
	global mark
	mark := tmpClipFile.readUInt()
	FileDelete, %clipFile%
	if (mark = 591278) {	; Hex: AE 05 09 00 
		return true
	} else {
		return false
	}
